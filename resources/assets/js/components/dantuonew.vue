<template>
    <div class="text-center">
      ...
      <div class="ballholder danholder" @click="changeSelectDan">
        ...
        <div class="ballholder tuoholder waitdan2 " @click="changeSelectTuo">
        ...
        <div class="btn-block">
            ...
            <button class="btn btn-info confirm  btn-danger cannot"
                    @click="confirmSelect"
            >确认选择</button>
    ...
    export default {
        ...
        methods: {
            ...
            changeSelectDan: function () {
                ...
                //     return null;
                // }
                if ($(event.target).hasClass('tuoused') ||$(event.target).parent('div.circlebai').hasClass('tuoused')) { //是否使用过胆球功能
                    alert('此号码已被拖码使用!');
                    return null;
                }
                ...
                let o = $('span.valuehere').data('danqiu')?$('span.valuehere').data('danqiu'):([]);
                //找出O.
                console.log('o==',o);
                ...
                if(tempdan.length >=1 && tempdan.length<=5) {
                    ...
                    if(   $('span.valuehere').data('tuook')==='yes' &&
                    ...
                    if(! $('button.confirm').hasClass('cannot'))
        ...
        changeSelectTuo: function () {
            ...
            //     return null;
            // }
            let temptuo = $('span.valuehere').data('temptuo')?$('span.valuehere').data('temptuo'):[];
            let tuofinal = [];
            let otuo = $('span.valuehere').data('otuo')?$('span.valuehere').data('otuo'):[];
            ...
            if ($(event.target).hasClass('circlebai')) {
                ...
                if(!$(event.target).hasClass('red')) {
                    let index = parseInt( $(event.target).find('span').text());
                    temptuo.push(index);
                    $('div.danholder div.cirlbai'+index).addClass('tuoused');
                }else {
                    let index = parseInt( $(event.target).find('span').text());
                    temptuo.remove(index);
                    $('div.danholder div.cirlbai'+index).removeClass('tuoused');
                }
                $(event.target).toggleClass('red');
                $(event.target).find('span').toggleClass('white');
            ...
            if ($(event.target).hasClass('numnum')) {
                ...
                if(!$(event.target).hasClass('white')) { //未变色时
                    let index = parseInt( $(event.target).text());
                    temptuo.push(index);
                    $('div.danholder div.cirlbai'+index).addClass('tuoused');
                }else {
                    let index = parseInt( $(event.target).text());
            ...
            if(temptuo.length >=2 ) {
                ...
                if(   $('span.valuehere').data('danok')==='yes' &&
                    $('span.valuehere').data('blueok')==='yes'
                ){
                    $('button.confirm').removeClass('cannot');
                }
        ...
        changeSelectBlue: function () {
                let tempblue = $('span.valuehere').data('tempblue')?$('span.valuehere').data('tempblue'):[];
            if ($(event.target).hasClass('circlebai')) {
        ...
        confirmSelect: function () {
            this.$router.push('confirm?type=dantuo', onComplete => {
